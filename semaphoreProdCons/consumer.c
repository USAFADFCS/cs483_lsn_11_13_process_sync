...
int main() {
    ...
    //Add const char for the read and write semaphores
    int shm_fd; //Shared memory file descriptor
    void *ptr; //Shared memory location that moves
    void *buf_start; //Track the beginning of the shared buffer
    sem_t *sem_ptr_num_write; //Ready to write to guessed number buffer
    //TODO: Add semaphore pointers for read and write protection for the number guessed
    int notWinning = 1;
    int userGuess; //Number guessed by the user, received from the shared buffer
    char userGuessString[50]; //Buffer to read from the shared memory location
    srand(time(NULL)); //Create a random number between 1 and 15.
    ...
    //TODO: Create shared semaphores and link to your already declared semaphore pointers
    ...
    //printf("%s\n", (char *)ptr);
    // Game code Use this code to help you build the producer. You may modify if you want to implement differntly.
